  ...
  try {
    ...
    await contests.forEachAsync(async contest => {
      ...
      await contest.problems.split('|').forEachAsync(async x => {
        ...
        total = total + 1;
        if (judge_state && judge_state.status === 'Accepted') count = count + 1;
      })
      contest.count = count;
      contest.total = total;
    })
...
app.get('/contest/:id/edit', async (req, res) => {
  try {
    ...
    if (!res.locals.user || contest && !await contest.isSupervisior(res.locals.user)) throw new ErrorMessage('您没有权限进行此操作。');
...
app.post('/contest/:id/edit', async (req, res) => {
  try {
    ...
    if (!['noi', 'ioi', 'acm', 'prac'].includes(req.body.type)) throw new ErrorMessage('无效的赛制。');
    ...
    if (player) {
      for (let problem of problems) {
        if (contest.isEnded()) {
          ...
          problem.judge_id = judge_state.id;
        ...
        } else if (contest.type === 'acm' || contest.type === 'prac') {
          if (player.score_details[problem.problem.id]) {
            problem.status = {
      ...
      for (let problem of problems) {
        ...
        for (let player of players) {
          if (player.score_details[problem.problem.id]) {
            ...
            if (((contest.type === 'acm' || contest.type === 'prac') && player.score_details[problem.problem.id].accepted) || ((contest.type === 'noi' || contest.type === 'ioi') && player.score_details[problem.problem.id].score === 100)) {
...
app.get('/contest/:id/problem/:pid', async (req, res) => {
  try {
    ...
    if (state && state.submit_time < contest.start_time) state = null;
