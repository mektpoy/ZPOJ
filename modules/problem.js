...
app.get('/problems', async (req, res) => {
  try {
    ...
    if (!['id', 'title', 'rating', 'ac_num', 'submit_num', 'ac_rate',
          'publicize_time']
             .includes(sort) ||
    ...
    if (!res.locals.user ||
      ...
      if (res.locals.user) {
        where = {$or: {is_public: 1, user_id: res.locals.user.id}};
      } else {
        where = {is_public: 1};
      }
    }
    ...
    let paginate = syzoj.utils.paginate(
        await Problem.count(where), req.query.page, syzoj.config.page.problem);
    ...
    res.render('problems', {
      allowedManageTag: res.locals.user &&
          await res.locals.user.hasPrivilege('manage_problem_tag'),
      problems: problems,
      paginate: paginate,
      curSort: sort,
      curOrder: order === 'asc'
...
app.get('/problems/search', async (req, res) => {
  try {
    ...
    const sort = req.query.sort || syzoj.config.sorting.problem.field;
    const order = req.query.order || syzoj.config.sorting.problem.order;
    if (!['id', 'title', 'rating', 'ac_num', 'submit_num', 'ac_rate'].includes(
            sort) ||
        !['asc', 'desc'].includes(order)) {
      throw new ErrorMessage('错误的排序参数。');
    }

    let where = {$or: {title: {$like: `%${req.query.keyword}%`}, id: id}};

    if (!res.locals.user ||
        !await res.locals.user.hasPrivilege('manage_problem')) {
      if (res.locals.user) {
        where = {
          $and: [where, {$or: {is_public: 1, user_id: res.locals.user.id}}]
        };
      } else {
        where = {$and: [where, {is_public: 1}]};
      }
    }
    ...
    let problems = await Problem.query(
        paginate, where,
        [syzoj.db.literal('`id` = ' + id + ' DESC'), [sortVal, order]]);
...
app.get('/problems/tag/:tagIDs', async (req, res) => {
  try {
    if (!res.locals.user || res.locals.user.name == '' ||
      ...
      throw new ErrorMessage('您没有登录或没有访问此OJ的权限，请联系管理员。');
    let tagIDs =
        Array.from(new Set(req.params.tagIDs.split(',').map(x => parseInt(x))));
    ...
    for (let tagID of tagIDs) {
      ...
      sql +=
          '`problem`.`id` IN (SELECT `problem_id` FROM `problem_tag_map` WHERE `tag_id` = ' +
          tagID + ')';
    }
    ...
    if (!res.locals.user ||
        !await res.locals.user.hasPrivilege('manage_problem')) {
      if (res.locals.user) {
        sql += 'AND (`problem`.`is_public` = 1 OR `problem`.`user_id` = ' +
            res.locals.user.id + ')';
      } else {
    ...
    let paginate = syzoj.utils.paginate(
        await Problem.count(sql), req.query.page, syzoj.config.page.problem);
    let problems = await Problem.query(
  ...
  try {
    ...
    if (req.body.isAdd == 'true') {
      let favorite = (await Favorite.query(
          'select * from favorite where problem_id = ' + problem_id +
          ' and user_id = ' + res.locals.user.id))[0];
      if (favorite) {
      } else {
      ...
      if (favorite) {
        await Favorite.query(
            'DELETE FROM favorite WHERE problem_id = ' + problem_id +
...
app.get('/favorites', async (req, res) => {
  try {
    ...
    let paginate = syzoj.utils.paginate(await Problem.query(
        'select COUNT(problem.id) from problem inner join favorite on favorite.user_id=' +
        res.locals.user.id + ' and favorite.problem_id = problem.id'));
    let problems = await Problem.query(
        'select problem.* from problem inner join favorite on favorite.user_id=' +
        res.locals.user.id +
        ' and favorite.problem_id = problem.id order by problem.' + sort + ' ' +
        order);
  ...
  try {
    ...
    if (problem.is_public || problem.allowedEdit) {
      await syzoj.utils.markdown(problem, [
        'description', 'input_format', 'output_format', 'example',
        'limit_and_hint'
      ]);
    } else {
      throw new ErrorMessage('您没有权限进行此操作。');
    ...
    let like = null;
    let favorite = await Favorite.count(
        'select * from favorite where problem_id = ' + id +
        ' and user_id = ' + res.locals.user.id)
    if (favorite) like = 1;

    let testcases = await syzoj.utils.parseTestdata(
        problem.getTestdataPath(), problem.type === 'submit-answer');
  ...
  try {
    ...
    if (!problem) {
      ...
      if (await res.locals.user.hasPrivilege('manage_problem')) {
        ...
        if (customID) {
          if (await Problem.fromID(customID))
    ...
    let newTagIDs = await req.body.tags.map(x => parseInt(x))
                        .filterAsync(async x => ProblemTag.fromID(x));
...
app.get('/problem/:id/import', async (req, res) => {
  try {
    ...
    res.render('problem_import', {problem: problem});
  } catch (e) {
    syzoj.log(e);
    res.render('error', {err: e});
  }
...
app.post('/problem/:id/import', async (req, res) => {
  try {
    ...
    if (!json.success)
      throw new ErrorMessage('题目加载失败。', null, json.error);
    ...
    if (validateMsg)
      throw new ErrorMessage('无效的题目数据配置。', null, validateMsg);

    await problem.save();

    let tagIDs = (await json.obj.tags.mapAsync(
                      name => ProblemTag.findOne({where: {name: name}})))
                     .filter(x => x)
                     .map(tag => tag.id);
    ...
    var promise = new Promise(function() {
      try {
        let data = await download(
            req.body.url +
            (req.body.url.endsWith('/') ? 'testdata/download' :
                                          '/testdata/download'));
        await fs.writeFileAsync(tmpFile.path, data);
        await problem.updateTestdata(
            tmpFile.path, await res.locals.user.hasPrivilege('manage_problem'));
        if (json.obj.have_additional_file) {
          let additional_file = await download(
              req.body.url +
              (req.body.url.endsWith('/') ? 'download/additional_file' :
                                            '/download/additional_file'));
          await fs.writeFileAsync(tmpFile.path, additional_file);
          await problem.updateFile(
              tmpFile.path, 'additional_file',
              await res.locals.user.hasPrivilege('manage_problem'));
        }
      } catch (e) {
        syzoj.log(e);
...
// The 'manage' is not `allow manage`'s 'manage', I just have no better name for
// it.
app.get('/problem/:id/manage', async (req, res) => {
  try {
    ...
    res.render('problem_manage', {problem: problem, testcases: testcases});
  } catch (e) {
    syzoj.log(e);
    res.render('error', {err: e});
  }
...
app.post(
    '/problem/:id/manage', app.multer.fields([
      {name: 'testdata', maxCount: 1}, {name: 'additional_file', maxCount: 1}
    ]),
    async (req, res) => {
      try {
        let id = parseInt(req.params.id);
        ...
        if (req.files['additional_file']) {
          await problem.updateFile(
              req.files['additional_file'][0].path, 'additional_file',
              await res.locals.user.hasPrivilege('manage_problem'));
        }
...
async function setPublic(req, res, is_public) {
  try {
    ...
    JudgeState.model.update({is_public: is_public}, {where: {problem_id: id}});
...
app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!problem) throw new ErrorMessage('无此题目。');
        if (problem.type !== 'submit-answer' &&
            !syzoj.config.enabled_languages.includes(req.body.language))
          throw new ErrorMessage('不支持该语言。');
        if (!curUser)
          throw new ErrorMessage('请登录后继续。', {
            '登录': syzoj.utils.makeUrl(
                ['login'], {'url': syzoj.utils.makeUrl(['problem', id])})
          });
        ...
        if (problem.type === 'submit-answer') {
          ...
          if (!req.files['answer']) {
            ...
            if (req.files['answer'][0].size > syzoj.config.limit.submit_answer)
              throw new ErrorMessage('答案文件太大。');
          ...
          if (size > syzoj.config.limit.submit_answer)
            throw new ErrorMessage('答案文件太大。');
          ...
          let code;
          if (req.files['answer']) {
            if (req.files['answer'][0].size > syzoj.config.limit.submit_code)
              throw new ErrorMessage('代码文件太大。');
            let fs = Promise.promisifyAll(require('fs'));
            code = (await fs.readFileAsync(req.files['answer'][0].path))
                       .toString();
          } else {
            if (req.body.code.length > syzoj.config.limit.submit_code)
              throw new ErrorMessage('代码太长。');
            code = req.body.code;
          }
        ...
        if (contest_id) {
          ...
          if ((!contest.isRunning()) && (!await contest.isSupervisior(curUser)))
            throw new ErrorMessage('比赛未开始或已结束。');
        ...
        if (problem.type !== 'submit-answer' &&
          ...
          let key = syzoj.utils.getFormattedCodeKey(
              judge_state.code, req.body.language);
          let formattedCode = await FormattedCode.findOne({where: {key: key}});

          if (!formattedCode) {
            let formatted = await CodeFormatter(
                judge_state.code, syzoj.languages[req.body.language].format);
            if (formatted) {
              formattedCode =
                  await FormattedCode.create({key: key, code: formatted});
...
app.post('/problem/:id/delete', async (req, res) => {
  try {
    ...
    if (!problem.isAllowedManageBy(res.locals.user))
      throw new ErrorMessage('您没有权限进行此操作。');
...
app.get('/problem/:id/testdata', async (req, res) => {
  try {
    ...
    res.render(
        'problem_data',
        {problem: problem, testdata: testdata, testcases: testcases});
  } catch (e) {
    syzoj.log(e);
    res.status(404);
    res.render('error', {err: e});
      ...
      try {
        ...
        if (req.files) {
          for (let file of req.files) {
            await problem.uploadTestdataSingleFile(
...
app.get('/problem/:id/testdata/download/:filename?', async (req, res) => {
  try {
    ...
    let filename = req.params.filename ?
        ...
        (problem.getTestdataArchivePath());
    if (!await syzoj.utils.isFile(filename))
      throw new ErrorMessage('文件不存在。');
...
app.get('/problem/:id/download/additional_file', async (req, res) => {
  try {
    ...
    res.download(
        problem.additional_file.getPath(), `additional_file_${id}.zip`);
  } catch (e) {
    syzoj.log(e);
    res.status(404);
    res.render('error', {err: e});
...
app.get('/problem/:id/statistics/:type', async (req, res) => {
  try {
    ...
    let paginate = syzoj.utils.paginate(
        count, req.query.page, syzoj.config.page.problem_statistics);
    ...
    res.render(
        'statistics',
        {statistics: statistics, paginate: paginate, problem: problem});
  } catch (e) {
    syzoj.log(e);
    res.render('error', {err: e});
  }
...
/*
app.post('/problem/:id/custom-test', app.multer.fields([{ name: 'code_upload',
maxCount: 1 }, { name: 'input_file', maxCount: 1 }]), async (req, res) => { try
{ let id = parseInt(req.params.id); let problem = await Problem.fromID(id);

    if (!problem) throw new ErrorMessage('无此题目。');
    if (!res.locals.user) throw new ErrorMessage('请登录后继续。', { '登录':
syzoj.utils.makeUrl(['login'], { 'url': syzoj.utils.makeUrl(['problem', id]) })
}); if (!await problem.isAllowedUseBy(res.locals.user)) throw new
ErrorMessage('您没有权限进行此操作。');

    let filepath;
    if (req.files['input_file']) {
      if (req.files['input_file'][0].size >
syzoj.config.limit.custom_test_input) throw new ErrorMessage('输入数据过长。');
      filepath = req.files['input_file'][0].path;
    } else {
      if (req.body.input_file_textarea.length >
syzoj.config.limit.custom_test_input) throw new ErrorMessage('输入数据过长。');
      filepath = await require('tmp-promise').tmpName({ template:
'/tmp/tmp-XXXXXX' }); await require('fs-extra').writeFileAsync(filepath,
req.body.input_file_textarea);
    }

    let code;
    if (req.files['code_upload']) {
      if (req.files['code_upload'][0].size > syzoj.config.limit.submit_code)
throw new ErrorMessage('代码过长。'); code = (await
require('fs-extra').readFileAsync(req.files['code_upload'][0].path)).toString();
    } else {
      if (req.body.code.length > syzoj.config.limit.submit_code) throw new
ErrorMessage('代码过长。'); code = req.body.code;
    }
